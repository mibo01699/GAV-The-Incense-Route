...
## 🔒 Security & Data Integrity Compliance
- **Anti-Phishing Enclosure:** All operations utilize the enclosed Pi payment overlay. No plain-text private keys are handled by the server.
- **Audit Trails:** Transaction identifiers (`txid`) from both the native Pi ledger and the YER service token ledger are bound to the tracking block of the physical asset (e.g., Yemeni Coffee shipments) to achieve 100% auditability for international oversight bodies.
